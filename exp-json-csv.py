...
#fuzzy logic search
for i in range(0,len(data)):
    data["fuzz1"].iloc[i] = lev.ratio(data["Contact Name"].iloc[i], data["Name1"].iloc[i])
    data["fuzz2"].iloc[i] = lev.ratio(data["Contact Name"].iloc[i], data["Name2"].iloc[i])
...
data.to_csv('process-report-oct9.csv',index=False)
